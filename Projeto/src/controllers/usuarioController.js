                    ...
                    if (resultadoAutenticar.length == 1) {
                        console.log(resultadoAutenticar);
                                    res.json({
                                        ...
                                        contrato: resultadoAutenticar[0].contrato
                                    });
...
function cadastrar(req, res) {
    ...
    if (nome == undefined) {
        ...
        usuarioModel.cadastrar(nome, email, senha, telefone, fkFilial)
            .then(
                function (resultado) {
                    res.json(resultado);
                }
...
function consultar(req, res) {
    var idUsuario = req.body.idFuncionarioServer;
    var senha = req.body.senhaServer;
    ...
    if (idUsuario == undefined) {
        ...
        usuarioModel.consultar(idUsuario, senha)
            .then(
                function (resultadoAutenticar) {
                    console.log(`\nResultados encontrados: ${resultadoAutenticar.length}`);
                    console.log(`Resultados: ${JSON.stringify(resultadoAutenticar)}`); // transforma JSON em String
...
function alterar(req, res) {
    ...
    if (idUsuario == undefined) {
        ...
        // Passe os valores como parâmetro e vá para o arquivo usuarioModel.js
        usuarioModel.alterar(idUsuario, senha)
            .then(
                function (resultado) {
                    res.json(resultado);
                }
